  ...
  query: z
    .object({
      ...
      lastModifiedAt: z
        .string()
        .optional()
        .describe('Filter salesOrders modified after this date in ISO format'),
      environmentName: z
        .string()
        .optional()
      ...
      status: z.enum(['OPEN', 'CLOSED']).optional().describe('Filter salesOrders by status'),
    })
    .optional(),
});
...
export const apiTool = {
  ...
  run: async ({ headers, query }: z.infer<typeof inputSchema>) => {
    ...
    try {
      ...
      const mapped = data.data.map((salesOrder: any) => ({
        id: salesOrder.id,
