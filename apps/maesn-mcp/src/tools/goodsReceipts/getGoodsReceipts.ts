  ...
  query: z
    .object({
      ...
      lastModifiedAt: z
        .string()
        .optional()
        .describe('Filter goods receipts modified after this date in ISO format'),
      environmentName: z
        .string()
        .optional()
...
export const apiTool = {
  ...
  description: 'Get a list of goods receipts',
  input: inputSchema,
  run: async ({ headers, query }: z.infer<typeof inputSchema>) => {
    ...
    try {
      ...
      const mapped = data.data.map((goodsReceipt: any) => ({
        id: goodsReceipt.id,
        comment: goodsReceipt.comment,
