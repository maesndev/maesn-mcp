...
const lineItemSchema = z.object({
  ...
  description: z.string().describe("Description of the purchased product or service").optional(),
  dimensions: z.array(dimensionSchema).default([]).describe("List of dimensions such as cost centers for this line item").optional(),
  itemId: z.string().describe("Unique identifier of the purchased item").optional(),
  itemName: z.string().describe("Short name or label of the purchased item").optional(),
  quantity: z.number().describe("Quantity of units purchased for this line item").optional(),
...
const inputSchema = z.object({
  ...
  body: z
    .object({
      accountId: z.string().describe("Identifier of the main account associated with the bill").optional(),
      addresses: z.array(addressSchema).default([]).describe("List of addresses associated with the bill").optional(),
      billDate: z.string().describe("Date when the bill was issued, in ISO 8601 format").optional(),
      billNumber: z.string().describe("Unique number or identifier assigned to the bill").optional(),
      contactId: z.string().describe("Identifier of the contact or supplier linked to the bill").optional(),
      currency: z.string().describe("Currency code (ISO 4217) of the bill").optional(),
      deliveryDate: z.string().describe("Date when goods or services were delivered").optional(),
      dueDate: z.string().describe("Date when the bill is due for payment, in ISO 8601 format").optional(),
      fileId: z.string().describe("Identifier of an attached file such as a scanned bill").optional(),
      journalCode: z.string().describe("Journal code where the bill is recorded").optional(),
      lineItems: z.array(lineItemSchema).default([]).describe("Detailed list of purchased products or services").optional(),
      name: z.string().describe("Title or label of the bill, often a supplier name").optional(),
      oneLineAddress: z.string().describe("Full address provided as a single line of text").optional(),
      paidDate: z.string().describe("Date when the bill was paid, in ISO 8601 format").optional(),
      paymentDays: z.number().describe("Number of days allowed for payment").optional(),
      paymentStatus: z.string().describe("Current payment status, e.g., PENDING or PAID").optional(),
      paymentTermId: z.string().describe("Identifier of the payment term applied to the bill").optional(),
      reference: z.string().describe("Reference number or external identifier of the bill").optional(),
      ...
      status: z.string().describe("Current status of the bill, e.g., DRAFT or APPROVED").optional(),
      ...
      totalDiscountAmount: z.number().describe("Total discount amount applied to the bill").optional(),
      totalDiscountPercentage: z.number().describe("Overall discount percentage applied to the bill").optional(),
      totalGrossAmount: z.number().describe("Total gross amount including tax for the bill").optional(),
      totalNetAmount: z.number().describe("Total net amount excluding tax for the bill").optional(),
      totalTaxAmount: z.number().describe("Total tax amount applied to the bill").optional(),
    })
    .describe('The data of the bill you want to create ').default({}),
});

export const apiTool = {
  name: 'putBill',
  description: 'Update a bill with the PUT command. This means you replace the whole bill object with a new one',
  input: inputSchema,
