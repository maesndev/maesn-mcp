  ...
  query: z
    .object({
      ...
      lastModifiedAt: z
        .string()
        .optional()
        .describe('Filter items modified after this date in ISO format'),
      environmentName: z
        .string()
        .optional()
      ...
      type: z.enum(['PRODUCT', 'SERVICE']).optional().describe('Filter items by type, choose between product or service'),
      itemNumber: z
        .string()
        .optional()
        .describe('Filter items by payment status'),
    })
    .optional(),
});
...
export const apiTool = {
  ...
  run: async ({ headers, query }: z.infer<typeof inputSchema>) => {
    const url = new URL(
      `https://unified-backend-prod.azurewebsites.net/accounting/items`
    );
    if (query?.pagination) {
    ...
    if (query?.itemNumber) url.searchParams.append('itemNumber', query.itemNumber);
