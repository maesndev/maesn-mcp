  ...
  query: z
    .object({
      ...
      lastModifiedAt: z
        .string()
        .optional()
        .describe('Filter payment terms modified after this date in ISO format'),
      environmentName: z
        .string()
        .optional()
      ...
      status: z.string().optional().describe('Filter payment terms by status'),
      paymentStatus: z
        .string()
        .optional()
        .describe('Filter payment terms by payment status'),
    })
    .optional(),
});
...
export const apiTool = {
  ...
  description: 'Get a list of paymentTerms',
  input: inputSchema,
  run: async ({ headers, query }: z.infer<typeof inputSchema>) => {
    const url = new URL(
      `https://unified-backend-prod.azurewebsites.net/accounting/paymentTerms`
    );
    if (query?.pagination) {
    ...
    try {
      ...
      const mapped = data.data.map((paymentTerm: any) => ({
        id: paymentTerm.id,
